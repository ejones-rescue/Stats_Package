...
# %%
#df_Baseline= df[df['Project_Wave'] == 'Baseline']
#df_Benchmark= df[df['Project_Wave'] == 'Benchmark']
#df_FU_1= df[df['Project_Wave'] == 'FU_1']
#df_FU_2= df[df['Project_Wave'] == 'FU_2']

# %%
##creating dfs 
#def IV_dfs(df,IV, IV_label):
#    y = df[df[IV]==IV_label]
#    return y

# %%
#Base_df = IV_dfs(df, 'Project_Wave', 'Baseline')
#FU1_df = IV_dfs(df, 'Project_Wave', 'FU_1')
#FU2_df = IV_dfs(df, 'Project_Wave', 'FU_2')

# %%
##creating dfs GENERATOR
#def IV_dfs(df,IV, IV_labels):
#    for x in IV_labels:
#        y = df[df[IV]==x]
#    yield y

# %%
#IV_labels = ['Benchmark', "Baseline",'FU_1']
#dfswow = IV_dfs(df,'Project_Wave',IV_labels)

# %%
#print(next(dfswow))

# %%
#df_names = [df_bench, df_base, df_fu1]
#
#for x,y in zip(dfswow, df_names):
#    y = x
#    print(y.shape)
#    
#    
...
# %%
#for x in df_list:
#    scipy.stats.kruskal(df[DV].dropna(), df[IV].dropna())

# %%
##Two sample t-test function, just one test, not user facing
#def KW_1(df_list, DV, IV, IV_labels):
#    statistic, pval = scipy.stats.kruskal(df[DV].dropna(), df[IV].dropna())
#    ttest_df = pd.DataFrame(data = {'statistic': t_statistic, 'p value': pval}, index = [0])
#    ttest_df['DV'] = DV
#    ttest_df['IV'] = IV
#    return ttest_df

# %%
#kw_vw_wave = scipy.stats.kruskal(df_Baseline['Vape_Would_Composite'],df_FU_1['Vape_Would_Composite'],df_FU_2['Vape_Would_Composite'])
#

# %%
#rar = df_Baseline['Vape_Would_Composite'],df_FU_1['Vape_Would_Composite'],df_FU_2['Vape_Would_Composite']

# %%
#IV_labels.count()

# %%
#IV_labels = ['Benchmark', "Baseline"]
#for x in IV_labels:
#    print (df[df['Project_Year'] == x])
#
